...
#  Идеи:
# Авивки. За ачивки можно начислять баллы.
...
# Пройти в общем: 10.000/100.000/1.000.000 шагов
# Пройти в общем км: 1/10/50/100/500/1.000/5.000/10.000 км
# Пройти: 1/5/10/25/42.195/50/100 км за один день.
...
# Баги:
# Не правильно считает кол-во дней, в которые пройдено более 10к шагов
...
STEPS = steps + [10204, 10039, 8469, 7859, 10725, 10170, 10444, ]
...
### !!!!! Этот цикл не правильно работает, нужно переписать. Он отображает данные, которых нет.
...
#            for row in minute_all_time:
#                # Более быстрый метод расчёта. Работает, примерно, в 1.5 раза быстрее, чем более компактный метод выше.
#                if int(row['time'][:-3]) == 0:
...
# Старая часть кода. Ее заменил на более простой код.
#        for row in file_reader_steps_copy:
#            # Забираем данные по месяцам
#            if row['date'][5:-3] == '01':
