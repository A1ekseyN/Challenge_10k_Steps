...
settings_steps_graph = False        # Отображение графика со статистикой по шагам
setting_days_week_average = False    # Статистика для отображение среднего кол-ва шагов на протяжения дня недели
setting_hour_statistics = True     # Статистика по кол-ву шагов в конкретный час дня.
setting_month_year_stat_steps = False     # Статистика по кол-ву шагов по месяцам
...
STEPS = steps + [10204, 10039, 8469, 7859, ]
...
if setting_hour_statistics == True:     # Настройка вкл/выкл расчёта статистики по часам
...
def mounth_year_stat_steps():
    ...
    with open("ACTIVITY_STEPS_ALL_TIME.csv") as file_steps_all_time_csv:
        ...
        for row in file_reader_steps_copy:
            ...
            if row['date'][5:-3] == '05':
                month_stat_steps['май'].append(int(row['steps']))
            if row['date'][5:-3] == '06':
                month_stat_steps['июнь'].append(int(row['steps']))
            if row['date'][5:-3] == '07':
                month_stat_steps['июль'].append(int(row['steps']))
            if row['date'][5:-3] == '08':
                month_stat_steps['август'].append(int(row['steps']))
            if row['date'][5:-3] == '09':
                month_stat_steps['сентябрь'].append(int(row['steps']))
            if row['date'][5:-3] == '10':
                month_stat_steps['октябрь'].append(int(row['steps']))
            if row['date'][5:-3] == '11':
                month_stat_steps['ноябрь'].append(int(row['steps']))
            if row['date'][5:-3] == '12':
                month_stat_steps['декабрь'].append(int(row['steps']))

        for row in file_reader_steps_copy:
            # Забираем данные по годам 2018-2022 года.
            if row['date'][:-6] == '2018':
                year_stat_steps['2018'].append(int(row['steps']))
            if row['date'][:-6] == '2019':
                year_stat_steps['2019'].append(int(row['steps']))
            if row['date'][:-6] == '2020':
                year_stat_steps['2020'].append(int(row['steps']))
            if row['date'][:-6] == '2021':
                year_stat_steps['2021'].append(int(row['steps']))
            if row['date'][:-6] == '2022':
                year_stat_steps['2022'].append(int(row['steps']))
...
def bar_month_year_stat_steps():
    ...
    y_steps_average = (
        int(sum(year_stat_steps['2018']) / len(year_stat_steps['2018'])),
        int(sum(year_stat_steps['2019']) / len(year_stat_steps['2019'])),
        int(sum(year_stat_steps['2020']) / len(year_stat_steps['2020'])),
        int(sum(year_stat_steps['2021']) / len(year_stat_steps['2021'])),
        int(sum(year_stat_steps['2022']) / len(year_stat_steps['2022']))
    )
...
print(f"В среднем, в день: {Fore.LIGHTGREEN_EX}{steps_average:,.0f}{Style.RESET_ALL} шагов. (Запас: {Fore.LIGHTGREEN_EX}{stock_steps_to_10k_per_day:,.0f}{Style.RESET_ALL} шага).")
...
if setting_days_week_average == True:
...
if setting_hour_statistics == True:
...
if setting_month_year_stat_steps == True:
...
if settings_steps_graph == True:        # Построение графика по количеству шагов.
    steps_graph()
if setting_days_week_average == True:   # Гистограмма которая показывает статистику по шагам на протяжении недели.
    bar_days_week_average()
if setting_hour_statistics == True:     # Гистограмма для отображения статистики по шагам в разные часы, на протяжении дня.
    bar_hours_stat_steps()
if setting_month_year_stat_steps == True:    # Гистограмма среднего кол-ва шагов за месяц.
