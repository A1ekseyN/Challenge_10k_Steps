...
print(f"В среднем, в день: {Fore.GREEN}{steps_average:,.0f}{Style.RESET_ALL} шагов. (Запас {stock_steps_to_10k_per_day} шагов).")
...
print(f"\n--- Сколько шагов в среднем пройдено в каждый день недели: ---")
print(f"\t- Понедельник: {day_monday_average}; (min/max: {min(day_monday)}/{max(day_monday)}).")
print(f"\t- Вторник: {day_tuesday_average}; (min/max: {min(day_tuesday)}/{max(day_tuesday)}).")
print(f"\t- Среда: {day_wednesday_average}; (min/max: {min(day_wednesday)}/{max(day_wednesday)}).")
print(f"\t- Четверг: {day_thursday_average}; (min/max: {min(day_thursday)}/{max(day_thursday)}).")
print(f"\t- Пятница: {day_friday_average}; (min/max: {min(day_friday)}/{max(day_friday)}).")
print(f"\t- Суббота: {day_saturday_average}; (min/max: {min(day_saturday)}/{max(day_saturday)}).")
print(f"\t- Воскресение: {day_sunday_average}; (min/max: {min(day_sunday)}/{max(day_sunday)}).")
